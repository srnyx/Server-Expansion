...
public class TickListener implements Listener {
    ...
    public void event(ServerTickEndEvent event) {
        ...
        for (double d : this.durations1s.values()) totalOneSecond += d;
        for (double d : this.durations10s.values()) totalTenSeconds += d;
        for (double d : this.durations1m.values()) TotalOneMinute += d;
